...
with tempfile.TemporaryDirectory() as tmp:
    ...
    manifest.write_text("sample_id\tcondition\nS1\tControl\nS2\tCRC\n", encoding="utf-8")
    for sample in ("S1", "S2"):
    ...
    for sample in ("S1", "S2"):
        for profiler in ("kraken2_bracken", "metaphlan4"):
            data.append(["feng", sample, "community", profiler, "1"])
    for profiler in ("kraken2_bracken", "metaphlan4"):
        data.append(["feng", "S1", "independent", profiler, "1"])
    ...
    out = root / "out"
    command = [sys.executable, str(repo / "analysis_v2/scripts/check_cohort_definitive_readiness.py"),
        "--cohort", "feng", "--manifest", str(manifest), "--state-dir", str(state), "--canonical", str(canonical),
...
print("[PASS] CRC cohort definitive-readiness fixture")
