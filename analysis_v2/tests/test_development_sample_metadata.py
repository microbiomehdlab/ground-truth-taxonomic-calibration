...
with tempfile.TemporaryDirectory() as tmp:
    root = Path(tmp); canonical = root/"canonical.tsv"; feng=root/"feng.tsv"; zeller=root/"zeller.tsv"; yachida=root/"yachida.tsv"; output=root/"metadata.tsv"
    ...
    yachida.write_text("sample_id\tTarget_Condition\tStudy\tage\tsex\tbmi\nY1\tAdenoma\tYachidaS_2019\t55\tFemale\t23\n",encoding="utf-8")
    subprocess.run(["python3",str(script),"--canonical",str(canonical),"--feng-manifest",str(feng),"--zeller-manifest",str(zeller),"--yachida-manifest",str(yachida),"--output",str(output)],check=True)
    with output.open(newline="",encoding="utf-8") as handle: rows=list(csv.DictReader(handle,delimiter="\t"))
    assert [row["sample_id"] for row in rows]==["F1","Y1","Z1"]; assert output.with_suffix(".tsv.sha256").is_file()
