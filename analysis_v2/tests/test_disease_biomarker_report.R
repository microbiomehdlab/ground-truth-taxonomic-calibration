...
metrics$target_called <- 1; metrics$enriched_calls <- 2; metrics$off_target_enriched_calls <- 1
metrics$precision <- .5; metrics$recall <- 1; metrics$f1 <- 2/3; metrics$target_effect <- .4
...
write.table(metrics, file.path(run, "evaluation", "biomarker_propagation_metrics.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)
...
stopifnot(file.exists(file.path(out, "SUCCESS")), file.exists(file.path(out, "figures", "target_recall.pdf")),
