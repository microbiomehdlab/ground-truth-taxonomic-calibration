#!/usr/bin/env Rscript
# Performance-only fork of fit_paired_biomarker_models.R for local runs on
# large inputs. Statistics are byte-for-byte identical to the canonical
# script; only the abundance lookup is pre-indexed once (a hash keyed by
# "profiler||source_profile") instead of re-filtering the full abundance
# frame inside the per-family, per-record loop. Not a substitute for the
# canonical script in a sealed/definitive run.
...
manifest <- as.data.frame(vroom(manifest_path, delim = "\t", col_types = vroom::cols(.default = "c"),
                                progress = FALSE))
abundance <- as.data.frame(vroom(abundance_path, delim = "\t", col_types = vroom::cols(.default = "c"),
                                 progress = FALSE))
...
manifest <- manifest[manifest$include == "1" | manifest$include == 1, , drop = FALSE]
...
# --- Performance change starts here: pre-index abundance once ---
abundance_key <- paste(abundance$profiler, abundance$source_profile, sep = "||")
abundance_split <- split(abundance, abundance_key)
lookup <- function(profiler, source_profile) {
  key <- paste(profiler, source_profile, sep = "||")
  hit <- abundance_split[[key]]
  if (is.null(hit)) hit[0, ] else hit
}
# --- Performance change ends here; everything below preserves the
#     canonical script's logic and output exactly. ---
...
for (fam_idx in seq_len(n_families)) {
  ...
  family_hits <- do.call(rbind, lapply(seq_len(nrow(family)), function(i)
    lookup(family$profiler[i], family$source_profile[i])))
  species <- sort(unique(family_hits$feature[family_hits$profiler == first$profiler]))
  ...
  matrix_for <- function(records) {
    ...
    for (i in seq_len(nrow(records))) {
      hits <- lookup(records$profiler[i], records$source_profile[i])
  ...
  message(sprintf("[%d/%d] family done: %s", fam_idx, n_families,
                  paste(unlist(first[1L, family_columns]), collapse = "/")))
}
...
writeLines(c("analysis\tpaired_perturbation_feature_models", "inference_unit\tbiological_sample",
             "status\tPASS", "generator\tfit_paired_biomarker_models_fast.R (local performance fork)"),
