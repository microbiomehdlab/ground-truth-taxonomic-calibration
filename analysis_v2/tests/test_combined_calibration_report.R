...
for (cohort in c("feng", "zeller")) for (population in c("independent", "community")) {
  ...
  slopes <- data.frame(profiler=c("kraken2_bracken","metaphlan4"),response_slope=c(.9,1.2),lower_95=c(.8,1.1),upper_95=c(1,1.3),deviation_from_one=c(-.1,.2),p_value_vs_one=c(.1,.01),q_value_bh_vs_one=c(.1,.02))
...
stopifnot(file.exists(file.path(out,"SUCCESS")),file.exists(file.path(out,"tables","detection_profiler_contrasts.tsv")),file.exists(file.path(out,"tables","detection_summary.tsv")),file.exists(file.path(out,"tables","calibration_interpretation.tsv")),file.exists(file.path(out,"tables","cohort_consistency.tsv")),file.exists(file.path(out,"figures","detection_probability.pdf")),file.exists(file.path(out,"figures","quantitative_response_slopes.png")),file.exists(file.path(out,"provenance","report.sha256")))
