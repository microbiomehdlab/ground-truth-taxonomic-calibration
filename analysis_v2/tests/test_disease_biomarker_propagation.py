...
with tempfile.TemporaryDirectory() as tmp:
    ...
    fields=["cohort","study","analysis_population","target_label","assembly_arm","profiler","contrast","dose_level","spike_fraction_target","feature","effect","p_value","q_value","include","exclusion_reason"]
    rows=[]
    for dose in (0,.01):
      values={"Target":(.2,.01 if dose else .2),"Retained":(.5,.01),"Lost":(.4,.2 if dose else .01),"Gained":(-.3,.01 if dose else .2)}
      for feature,(effect,q) in values.items(): rows.append(["yachida","S","independent","Pana","clean","metaphlan4","CRC_vs_Control","baseline" if dose == 0 else "dose_01",dose,feature,effect,q*q,q,1,""])
