#!/bin/bash
# Download FengQ_2015 FASTQ files from EBI, organized by Target Condition
#
# Usage:
#   ./download_fengq2015.sh <output_dir>
#
# Arguments:
#   <output_dir>     Root directory where condition subdirectories will be created
#
# Expected in the same directory as this script:
#   - Public_study__FengQ_2015.tsv   (metadata with 'Name' and 'Target Condition' columns)
#   - fengq_wgets.sh                 (wget script with '# Sample: <name>' blocks)
#
# Output structure:
#   <output_dir>/
#       fengq/
...
METADATA_TSV="$SCRIPT_DIR/Public_study__FengQ_2015.tsv"
WGET_SCRIPT="$SCRIPT_DIR/fengq_wgets.sh"
