...
This one-page SVG brings the existing matched-species and spike-to-biomarker audits together. It displays, for each cohort and profiler: unspiked detection in Control/Adenoma/CRC; adenoma positive-only median abundance and all-sample IQR; adjusted adenoma and CRC effects with 95% CIs and BH q; **two equal-community-spike call tests, at 0.001% and 0.1% per member**; and paired rescue and quantitative recovery at both community doses. The separate 0.01% single-species recovery experiment is deliberately omitted from this call-focused figure. An optional paired related-*Fusobacterium* response appears only if the low-dose source audit included `--paired-features`.
